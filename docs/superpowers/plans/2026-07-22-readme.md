...
**Goal:** Provide an accurate, practical README for setting up and using the local RAG workspace.

**Architecture:** Replace the existing short README with one self-contained Markdown guide. Derive commands, URLs, and supported formats directly from `compose.yaml`, `.env.example`, `scripts/validate_env.py`, and `app/services/storage.py`.

**Tech Stack:** Markdown, Docker Compose, Python/pytest, Vite/Vitest.

## Global Constraints

- Document only functionality implemented in the repository.
- Link to `.env.example` instead of duplicating its full configuration.
- Keep the guide focused on local setup and development.

---

### Task 1: Replace the project README

**Files:**
- Modify: `README.md`
- Verify: `compose.yaml`, `.env.example`, `scripts/validate_env.py`, `app/services/storage.py`, `pyproject.toml`, `frontend/package.json`

**Interfaces:**
- Consumes: Docker Compose services and environment variables defined in the listed configuration files.
- Produces: A Markdown setup guide for local users and contributors.

- [ ] **Step 1: Replace `README.md` with the documented setup guide**

Include these sections and exact commands:
...
- [ ] **Step 2: Verify Markdown and configuration references**

Run: `git diff --check && docker compose config --quiet`

Expected: exit status 0; no whitespace errors; Compose resolves the documented services and ports.
